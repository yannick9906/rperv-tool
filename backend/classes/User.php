<?php
    ...
    class User implements \JsonSerializable {
        ...
        public static function fromUID($uID) {
            $pdo = new PDO_MYSQL();
            $res = $pdo->query("SELECT * FROM rperv_user WHERE uID = :uid", [":uid" => $uID]);
        ...
        public static function fromUName($uName) {
            $pdo = new PDO_MYSQL();
            $res = $pdo->query("SELECT * FROM rperv_user WHERE username = :uname", [":uname" => $uName]);
            return new User($res->uID, $res->username, $res->passhash, $res->email, $res->endpoint);
        }
        ...
        public static function getList($page = 1, $pagesize = 75, $search = "", $sort = "") {
            ...
            $endElem = $pagesize;
            $stmt = $pdo->queryPagedList("rperv_user", $startElem, $endElem, ["username"], $search, $USORTING[$sort], "uID >= 0");
        ...
        public static function getListObjects($page = 1, $pagesize = 9999999, $search = "") {
            ...
            $startElem = ($page-1) * $pagesize;
            $endElem = $pagesize;
            $stmt = $pdo->queryPagedList("rperv_user", $startElem, $endElem, ["username"], $search, "", "uID >= 0");
            $hits = [];
            while($row = $stmt->fetchObject()) {
        ...
        public static function getListMeta($page, $pagesize, $search) {
            $pdo = new PDO_MYSQL();
            if($search != "") $res = $pdo->query("select count(*) as size from rperv_user where lower(concat(username)) like lower(concat('%',:search,'%')) and uID >= 0", [":search" => $search]);
            else $res = $pdo->query("select count(*) as size from rperv_user where uID >= 0");
            $size = $res->size;
        ...
        public function delete() {
            return $this->pdo->query("DELETE FROM rperv_user WHERE uID = :uid", [":uid" => $this->uID]);
        }
        ...
        public function saveChanges() {
            $this->pdo->queryUpdate("rperv_user",
         ...
         * @param string $email
         */
        public static function create($username, $passwdhash, $email) {
            $pdo = new PDO_MYSQL();
